...
export function HeatmapCard({ data }: { data: PredictionPayload }) {
  const imgB64 = data.segmentation_overlay_png_base64 ?? data.heatmap_png_base64;
  const isSeg = Boolean(data.segmentation_overlay_png_base64);
  if (!imgB64) {
    return (
    ...
    >
      <div className="border-b border-white/[0.06] px-3 py-2">
        <p className="text-[11px] font-semibold uppercase tracking-wider text-neuro-ice">
          {isSeg ? "Kanama maskesi örtüsü" : "Gradyan dikkat haritası"}
        </p>
        <p className="text-[10px] text-slate-500">
          {isSeg
            ? "U-Net segmentasyonu · kırmızı bindirme"
            : "Sınıfa koşullu saliency katmanı"}
        </p>
      </div>
