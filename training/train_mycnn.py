"""MyCNN ImageFolder eğitimi → kök: best_mycnn_v2.pth, grafikler: ../training_outputs/"""
import os
import copy
...
IMG_SIZE, BATCH_SIZE, LR, EPOCHS, PATIENCE = 224, 16, 5e-5, 20, 5
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(device, data_dir)
...
cw = torch.tensor([1.5, 1.0], device=device)
...
for epoch in range(EPOCHS):
    ...
    print(f"Ep{epoch+1} valL={val_losses[-1]:.4f}")
...
print("TEST", acc, prec, rec, cm)

pfx, ep = "mycnn", range(1, len(train_losses) + 1)
for name, a, b, ylab, ttl in (
    ("_train_val_loss.png", train_losses, val_losses, "Loss", "MyCNN — Loss"),
    ("_train_val_accuracy.png", train_accs, val_accs, "Accuracy", "MyCNN — Accuracy"),
):
...
ax.set_ylabel("True")
ax.set_title("MyCNN — Confusion (test)")
plt.tight_layout()
...
with mp.open("w", encoding="utf-8") as f:
    f.write("MyCNN (training/train_mycnn.py)\n")
